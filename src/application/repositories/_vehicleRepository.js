...
class VehicleRepository {
  ...
  async update({ marca, modelo, ano, placa, vehicleId, usuarioId }) {
    ...
    const updatedVehicle = await prismaClient.veiculo.update({
      ...
      data: {
        ...
        connect: { id: usuarioId }, // Conecta o veículo ao usuário
        atividades: {
          create: {
            tipo: "VEICULO",
            acao: "ATUALIZADA",
  ...
  async showMaintenanceByUserId({ accountId }) {
    const allMaintenance = await prismaClient.registroDeManutencao.findMany({
      where: {
        veiculo: {
          usuarioId: accountId,
          ...
          include: { registroDeManutencao: true }
        },
      },
    });
    console.log("208", allMaintenance)
    return allMaintenance;
  };
